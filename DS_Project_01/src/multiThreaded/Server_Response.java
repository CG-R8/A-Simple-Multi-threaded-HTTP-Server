...
public class Server_Response {
	...
	public String success_reponse(String date, Date last_modified_date,
			long content_length, String content_type) {
	...
	public int sendStaticResource(OutputStream output) throws IOException {
		...
		try {
			...
			//last_modified_date= java.time.format.DateTimeFormatter.RFC_1123_DATE_TIME
			//				.format(
			String content_type = new MimetypesFileTypeMap()
			...
			if (requestedfile.exists()) {
				...
				fis = new FileInputStream(requestedfile);
				int ch = fis.read(bytes, 0, 2048);
				byte[] b = success_reponse(date, last_modified_date,
						content_length, content_type).getBytes();
			...
			System.out.println("This is the error in server response."
					+ e.toString());
